...
3. 搜索关键词 → 获取帖子列表（强制等待 2-3 秒）
4. 去重+按点赞排序 → 选择高价值帖子
5. 依次获取帖子详情 → 每个帖子后等待 2-3 秒
6. 依次获取评论 → 每个帖子后等待 2-3 秒
7. 评论分析 → 先读帖再读评、识别阴阳怪气、去噪
8. 输出报告 → 自动匹配模板（可自行增减章节）
```
...
**违反频率限制的后果**：Chrome 调试器断开 → 所有后续 opencli 命令失败 → 需等待 10-15 分钟恢复 → 可能需要重启 Chrome 或重新登录。

### 反爬机制

- **请求频率**：每次操作强制间隔 2-3 秒，禁止并发；搜索命令也需要间隔 5-10 秒
- **URL要求**：必须使用搜索结果中的完整URL（含xsec_token），禁止 bare note ID；注意搜索结果中的 `search_result` URL 无法直接用于 note/comments 命令
- **错误处理**：SECURITY_BLOCK → 换用完整URL重试或停止等待 10-15 分钟；NOTE_NOT_FOUND → 跳过；连续3篇失败 → 降低频率或换方法
